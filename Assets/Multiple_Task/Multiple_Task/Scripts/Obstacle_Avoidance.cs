...
public class Obstacle_Avoidance : Agent
{
    ...
    public Transform walkerObstacle; 
    public float safeDistance = 0.35f; 
    ...
    private void RespawnTarget()
    {
        ...
        // 训练模式下直接传递精确坐标并加入微小噪声，跳过极其耗时的相机渲染
        if (isTrainingMode)
        {
    ...
    public override void CollectObservations(VectorSensor sensor)
    {
        ...
        // 避障状态：传入 Walker 的相对位置信息
        if (walkerObstacle != null)
        {
            Vector3 dirToWalker = walkerObstacle.position - currentTCP;
            sensor.AddObservation(dirToWalker.normalized); 
            sensor.AddObservation(dirToWalker.magnitude);  
        }
        else
        {
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            if (pauseCounter == 0) // 仅在非悬停状态下物理驱动关节
            {
        ...
        if (pauseCounter == 0) AddReward(-actionMagnitude * 0.0001f);

        // 2. 动态避障检测与基于梯度的惩罚 (Reward Shaping)
        if (walkerObstacle != null)
        {
            Vector2 walkerXZ = new Vector2(walkerObstacle.position.x, walkerObstacle.position.z);
            Vector2 tcpXZ_obs = new Vector2(currentTCP.x, currentTCP.z);
            float minDistToWalker = Vector2.Distance(tcpXZ_obs, walkerXZ);

            foreach (var joint in joints)
            {
                Vector2 jointXZ = new Vector2(joint.transform.position.x, joint.transform.position.z);
                float d = Vector2.Distance(jointXZ, walkerXZ);
                if (d < minDistToWalker) minDistToWalker = d;
            }

            // 计算物理残余速度
            float currentPhysicalSpeed = 0f;
            foreach (var joint in joints) currentPhysicalSpeed += Mathf.Abs(joint.jointVelocity[0]);

            float warningDistance = safeDistance + 0.6f; // 设置预警刹车区

            if (minDistToWalker < warningDistance)
            {
                // 活跃度 = 网络指令幅度 + 物理残余速度
                float activityLevel = actionMagnitude + currentPhysicalSpeed;

                if (minDistToWalker < safeDistance)
                {
                    // 【致死区】
                    if (activityLevel > 0.5f) // 依然在动，判定为肇事
                    {
                        SetReward(-100.0f); 
                        EndEpisode();
                        return;
                    ...
                    {
                        // 【成功悬停避让】
                        // 机械臂已老实停下，免除本帧的 stepPenalty (跳过后续代码)
                        // 这就告诉神经网络：“只要你在这里保持静止输出为0，就不会掉分”
                        return; 
                    }
                }
                else
                {
                    // 【预警区】
                    // Walker 越近，对动作的惩罚就越重。用负面奖励逼迫网络学会“踩刹车”
                    float dangerFactor = (warningDistance - minDistToWalker) / (warningDistance - safeDistance);
                    AddReward(-activityLevel * dangerFactor * 0.05f);
                }
            }
        }

        // ==========================================
        // 3. 目标距离与姿态计算 (原有的 stepPenalty 和 抓取状态机逻辑保留)
        // ==========================================
